...
public class SankeV2 {
	...
	private class Path implements Comparable<Path> {
		...
		/*
		 * public void printPath() { for (int i = 0; i < path.size() - 1; i++) {
		 * Node node = path.get(i); System.out.print("{" + node.x + ", " +
		 * node.y + ", " + node.value + "}" + " -> "); } System.out.print("{" +
		 * path.get(path.size() - 1).x + ", " + path.get(path.size() - 1).y +
		 * ", " + path.get(path.size() - 1).value + "}"); System.out.println();
		 * System.out.println(); Node start = path.get(path.size() - 1); Node
		 * end = path.get(0); System.out.println("Start Point = ( " + start.x +
		 * ", " + start.y + ") ... Elevation = " + start.value);
		 * System.out.println("End Point = ( " + end.x + ", " + end.y +
		 * ") ... Elevation = " + end.value); System.out.println(
		 * "Path length = " + pathSize); System.out.println("Score = " +
		 * currentScore); }
		 */
